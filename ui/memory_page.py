...
class MemoryPage(ctk.CTkFrame):
    ...
    def build_ui(self):
        ...
        subtitle = ctk.CTkLabel(
            header,
            text="Browse the activity JARVIS has remembered from your computer.",
            font=("Segoe UI", 14),
    ...
    def load_memories(self):
        ...
        self.count_label.configure(
            text=f"{len(rows)} recent memor{'y' if len(rows) == 1 else 'ies'}"
        )
        ...
        self.memory_box.tag_config("time", foreground=TEXT_DIM, font=("Segoe UI", 10))
        self.memory_box.tag_config("app", foreground=ACCENT, font=("Segoe UI", 12, "bold"))
        self.memory_box.tag_config("title", foreground=TEXT_BRIGHT, font=("Segoe UI", 12))
        self.memory_box.configure(state="disabled")
